...
# 確保 `uploads` 欄位存在於資料表中
try:
    c.execute("ALTER TABLE users ADD COLUMN uploads INTEGER DEFAULT 0")
    conn.commit()
...
def plot_gdp_comparison(df):
    ...
    ax1.plot(years, personal_gdp, marker='o', label='Personal GDP (Million)', color='orange')
    ax1.set_xlabel('Year')
    ax1.set_ylabel('GDP (Million(Million)')
    ax2 = ax1.twinx()
    ax2.plot(years, growth_rate, marker='o', label='Economic growth rate(%)', color='red')
    ax2.set_ylabel('Growth rate (%)', color='red')
    ax2.tick_params(axis='y', labelcolor='red')
...
if st.session_state['logged_in']:
    ...
    user_info = c.execute("SELECT uploads, payment FROM users WHERE username = ?", (st.session_state['username'],)).fetchone()
    if user_info is not None:
        uploads, payment = user_info
    else:
        uploads, payment = 0, 0
    
    if uploads >= 1 and not payment:
        st.warning("您已經達到免費上傳次數限制。請付費以繼續使用上傳功能。")
        show_payment_page()
    else:
        ...
        if uploaded_file is not None:
            try:
                ...
                if 'Year' in df_custom.columns:
                    ...
                    # 更新上傳次數
                    c.execute("UPDATE users SET uploads = uploads + 1 WHERE username = ?", (st.session_state['username'],))
                    conn.commit()
                else:
...
def show_payment_page():
    ...
    c.execute("UPDATE users SET payment = 1 WHERE username = ?", (st.session_state['username'],))
    conn.commit()
